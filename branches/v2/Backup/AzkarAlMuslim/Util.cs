       ...
       //public static extern int SetProcessWorkingSetSize(int hProcess, int dwMinimumWorkingSetSize, int dwMaximumWorkingSetSize);
        ...
        public static bool isConnectionAvailable(out string _returnMessage)
        {
            ...
            try
            {
                ...
                {
                    _success = false;
                    _returnMessage = "ãä ÝÖáß ÍÇæá Çä Êßæä ãÊÕáÇ ÈÇáÇäÊÑäÊ áßì íÊãßä ÇáÈÑäÇãÌ ãä ÊäÒíá ÇáÇÐßÇÑ ÇáÌÏíÏå ãä Úáì ÔÈßå ÇáãÚáæãÇÊ";
                   return _success;
                }
                else
                {
                    _returnMessage = "Êã ÇáÇÊÕÇá ÈÔÈßå ÇáãÚáæãÇÊ ÈäÌÇÍ ÍíË íãßäß ãä ÊäÒíá ÇáÇÐßÇÑ ÇáÌÏíÏå íæãíÇ ãä Úáì ÇáÔÈßå ÇáÚäßÈæÊíå";
                    _success = true;
                }
            }
